...
def clean_duplifix_file():
    try:
        # Lees alle berichten
        unique_messages = set()
        if os.path.exists('duplifix.txt'):
            with open('duplifix.txt', 'r', encoding='utf-8') as f:
                for line in f:
                    content = line.strip().split('|')[0].lower()
                    unique_messages.add(content)
        
        # Schrijf unieke berichten terug
        with open('duplifix.txt', 'w', encoding='utf-8') as f:
            for message in sorted(unique_messages):
                f.write(f"{message}\n")
        
        return unique_messages
    except Exception as e:
        print(f"Fout bij opschonen van duplifix.txt: {e}")
        return set()

def save_duplicate_message(message):
    with open('duplifix.txt', 'a', encoding='utf-8') as f:
        f.write(f"{message.content.strip()}\n")

def load_duplicate_messages():
    if not os.path.exists('duplifix.txt'):
        return set()
...
async def on_ready():
    ...
    known_duplicates = clean_duplifix_file()
        ...
        if len(messages) > 1:
            ...
            for duplicate in messages[1:]:
                try:
                    ...
                    print(f"Verwijderd duplicaat in {duplicate.channel.name} van {duplicate.author.name}")
